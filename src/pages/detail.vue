<template>
  ...
  <div class="detail" v-if="detail.title">
    <UcNav/>
    <div class="content">
      <div class="header clear"><h2><img :src="detail.img" alt=""/></h2><div class="chose"><p>购买阅读</p><p>支持本书</p></div></div>
      <div class="cont">
        <h3>{{detail.title}}</h3>
        <div class="time"><p>{{detail.time | date}}</p></div>
...
<style>
  ...
  .clear p{background: orangered; width: 1.25rem;}
  .chose{display: flex;flex-direction: column;justify-content: space-around;height: 1.8rem;}
</style>
